...
<div class="row">
    <g:form controller="companyAccountMessage" action="replyIn">
        ...
        <g:if test="${dialog.sender?.studentAccount?.cv}">

            <g:link controller="companyAccountCvSearch" action="getStudentCv" class="button secondary radius"
                    params="[id: dialog.sender?.studentAccount?.cv?.newFilename]" target="_blank">

                ${message(code: 'student.show.cv.label')}
            </g:link>
        </g:if>
